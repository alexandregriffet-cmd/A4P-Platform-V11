import { HUB_URL, PMP_URL } from '@/lib/constants'

export default function Page() {
  const portalBase = process.env.NEXT_PUBLIC_BASE_URL || ''
  const returnUrl = portalBase
    ? `${portalBase}/integration/receive?module=PMP`
    : `/integration/receive?module=PMP`
  const launchUrl = `${PMP_URL}?return_url=${encodeURIComponent(returnUrl)}`
