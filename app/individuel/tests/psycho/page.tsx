import { HUB_URL, PSYCHO_URL } from '@/lib/constants'

export default function Page() {
  const portalBase = process.env.NEXT_PUBLIC_BASE_URL || ''
  const returnUrl = portalBase
    ? `${portalBase}/integration/receive?module=PSYCHO`
    : `/integration/receive?module=PSYCHO`
  const launchUrl = `${PSYCHO_URL}?return_url=${encodeURIComponent(returnUrl)}`
